...
date: "2025-02-07"
description: "이번 포스팅에서는 렌더링 방식에 대해 정리한다. 렌더링은 사용성과 밀접한 부분이기 때문에 프론트엔드 개발자로서는 생명과도 같은 영역이다. 렌더링에 대해 잘 모르던 시절의 나를 질타하며 이번 정리글을 시작해본다. 렌더링 방식에 대해 따지기 앞서, 애플리케이션의 구조가 어떻게 바뀌어왔기에 그에 맞는 렌더링 방식이 필요해왔는지가 중요하다. 이는 브라우저 동작 방식에 대한 이야기는 아니다(브라우저 동작 방식은 다음 게시글로 다뤄볼까한다)."
...
이번 포스팅에서는 렌더링 방식에 대해 정리한다. 구체적으로는, 필자가 Next.js에 대해 처음 접하면서 정리한 렌더링에 대한 간단한 정리글 정도로 봐주면 좋겠다. 예를 들어, `SSG에 Google Analytics 4를 API Route를 통해 쓸려면 15.2의 App Router 방식에서는 뭘 써야한다`와 같이 구체적인 기술을 제시하기보다는 방법론적인 내용을 다룬다. 그렇다면, 렌더링 방식에 대해 따지기 앞서 애플리케이션의 구조가 어떻게 바뀌어왔기에 그에 맞는 렌더링 방식이 필요해왔는지가 중요하다. 이는 브라우저 동작 방식에 대한 이야기는 아니다(브라우저 동작 방식은 다음 게시글로 다뤄볼까한다).
...
SSR과 CSR을 조합하여 쓰는 방법과 실제 예제를 살펴보았다. SSG는 사용자에게 빠른 응답을 주기 위해, 페이지를 **빌드 시 미리 만들어두고 정적 파일로 제공하는 위하여 사용되는 렌더링 방식**이다. CSR의 단점인 초기 로딩 속도 문제와 SEO 최적화까지 모두 해결해주는 방식이다(주로 정적인 콘텐츠(블로그 글, 문서 등)에서 많이 사용되는 방식이며, 현재 기술 블로그가 렌더링 되는 방식이기도 하다).
